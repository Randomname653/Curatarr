...
let setupData = {};
let setupStep = 0;
const SETUP_STEPS = ['plex','ollama','metadata','arr','import','done'];
const CAT_LABELS = {music:'Music', movie:'Movies', show:'TV Shows', anime:'Anime'};
...
const SVG_WARN = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-2px;flex-shrink:0"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>';
const SVG_CHECK = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" style="vertical-align:-2px;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg>';
const SVG_X = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" style="vertical-align:-2px;flex-shrink:0"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
const SVG_TRASH = '<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>';
const SVG_STAR = '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" stroke="none" style="vertical-align:-1px;flex-shrink:0"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26"/></svg>';
...
function _posterImg(url, w, h, isMusic) {
...
window.onload = async () => {
  ...
  if (token) {
    try {
      ...
      // Only a genuine rejection (401/403) means the token itself is bad.
      ...
      // failure that had nothing to do with the token — now it just shows
      // the login screen for this load; the stored token survives so the
...
const SETUP_CONTENT = {
  ...
  ollama: () => `
    ...
    <div class="form-group" style="margin-top:12px">
      <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
        <input type="checkbox" id="s-pitcher-enable" onchange="document.getElementById('s-pitcher-wrap').style.display=this.checked?'':'none'" ${setupData.enable_pitcher?'checked':''}>
      ...
      <div id="s-pitcher-wrap" style="margin-top:8px;${setupData.enable_pitcher?'':'display:none'}">
  ...
  metadata: () => `
    ...
    <div class="form-group">
      ...
      <input id="s-omdb" placeholder="xxxxxxxx" value="${setupData.omdb_api_key||''}">
    ...
    <div class="form-group">
      ...
      <input id="s-soulsync-url" placeholder="http://192.168.1.100:12279" value="${setupData.soulsync_url||''}">
    ...
    <div class="form-group">
      ...
      <input id="s-soulsync-key" type="password" placeholder="xxxxxxxxxxxxxxxxxxxx" value="${setupData.soulsync_api_key||''}">
    </div>
    <div class="form-group">
      <label for="s-listenbrainz">ListenBrainz user token <span class="badge muted" style="font-size:10px">Optional · Music</span></label>
      <input id="s-listenbrainz" type="password" placeholder="free account token" value="${setupData.listenbrainz_token||''}">
      <div class="hint">Global listener counts as deletion evidence for music. Free account at <a href="https://listenbrainz.org/settings/" style="color:var(--amber)" target="_blank">listenbrainz.org/settings</a> — ListenBrainz requires the token since it auth-locked its API.</div>
    </div>`,
...
function collectStep() {
  ...
  if (key === 'plex') {
    ...
    setupData.base_curator_model = document.getElementById('s-curator-model')?.value || 'gemma4:31b';
    setupData.base_summarizer_model = document.getElementById('s-summarizer-model')?.value || 'granite4.1:8b';
    setupData.embedding_model = document.getElementById('s-embed-model')?.value || 'nomic-embed-text-v2-moe';
    setupData.enable_pitcher = !!document.getElementById('s-pitcher-enable')?.checked;
    setupData.base_pitcher_model = document.getElementById('s-pitcher-model')?.value || 'qwen3.8:27b';
    setupData.vram_gb = parseFloat(document.getElementById('s-vram')?.value) || setupData.vram_gb || null;
  } else if (key === 'metadata') {
    setupData.listenbrainz_token = document.getElementById('s-listenbrainz')?.value?.trim() || '';
    setupData.tmdb_api_key = document.getElementById('s-tmdb')?.value?.trim() || '';
    setupData.omdb_api_key = document.getElementById('s-omdb')?.value?.trim() || '';
    setupData.soulsync_url = document.getElementById('s-soulsync-url')?.value?.trim() || '';
    setupData.soulsync_api_key = document.getElementById('s-soulsync-key')?.value?.trim() || '';
...
async function testConn(service) {
  ...
  : '';
    ...
    if (service==='ollama' && r.ok && r.models) {
      setupData._detected_models = r.models;
      refreshModelRecs();
    }
  } catch(e) {
...
async function refreshModelRecs() {
  const vsel = document.getElementById('s-vram');
  if (vsel && vsel.value) setupData.vram_gb = parseFloat(vsel.value);
  const endpoint = document.getElementById('s-ollama')?.value?.trim() || setupData.ollama_endpoint || '';
  const panel = document.getElementById('model-recs');
  try {
    const r = await api('/api/setup/recommend', 'POST',
                        {ollama_endpoint: endpoint, vram_gb: setupData.vram_gb ?? null});
...
function renderRecSelect(id, entries, installed) {
...
async function uploadSpotify(files, p) {
...
async function runSpotifyImport(p) {
...
async function startPlexLogin() {
  ...
  const schedulePoll = (ms) => { clearInterval(pollInterval); pollInterval = setInterval(doPoll, ms); };
...
async function logout() {
  // Server first: bumping the user's token version revokes THIS token on
...
async function api(path, method='GET', body=null, _retried=false) {
  ...
  // 1/7th of its 7-day life on ANY response for a still-valid token -- ok or
  ...
  if (freshToken) { token = freshToken; localStorage.setItem('curatarr_token', freshToken); }
...
// deletions' _delProposalsAll tab cache) that a second generic layer here
...
async function _swrRun(key, fetchFn, renderFn, opts = {}) {
...
function _swrInvalidate(key) { _swrCache.delete(key); }
...
function menuHtml(items, label = 'More') {
...
function emptyHtml(html, ctaLabel, ctaCall, o = {}) {
...
function _fmtAbs(iso) { const dt = new Date(iso); return isNaN(dt.getTime()) ? '' : dt.toLocaleString(); }
...
function trackDirty(form, saveBtn) {
...
function _showTestResult(span, r) {
  if (!span) return;
...
function showView(name, btn) {
  ...
  if (!currentUser?.is_admin && ['deletions','curation','libraries','admin','reclassify','report'].includes(name)) return;
  ...
  if (name==='enrich') showKbTab(_kbTab);
  if (name==='recs') { if (_skipNextRecsLoad) { _skipNextRecsLoad = false; } else { loadRecs(currentRecsCategory); } }
...
async function writeYearlyReview(btn) {
...
function _renderReclassify(res) {
...
function _rcRow(from, to, changed) {
...
async function moveReclassify(btn) {
  ...
  const toolbar = `
    <div class="toolbar">
      ...
      <div class="toolbar-right">
        ...
        <label class="chip${state.needs_enrichment ? ' active' : ''}" title="Only items without a synopsis or with raw metadata still awaiting the polish">
          <input type="checkbox" ${state.needs_enrichment ? 'checked' : ''} onchange="setBrowserFilter('${svc}','${tabId}', this.checked)"> Needs enrichment</label>
        <input type="text" id="arr-search-${svc}-${tabId}" class="input" aria-label="Search items" value="${escAttr(state.search || '')}"
...
async function reEnrich(svc, arrId, mode) {
      ...
      <div class="section-body">
        <div class="row fs-12" style="gap:18px">
          ...
          <div class="row row-end">
            <label class="chip${state.only_resolved ? ' active' : ''}" title="Only artists whose MusicBrainz ID has been resolved by Phase 1.4 of the music pipeline — the ones with a clickable Add button.">
              <input type="checkbox" ${state.only_resolved ? 'checked' : ''} onchange="setBacklogOnlyResolved('${svc}', this.checked)"> Resolved only</label>
            <label class="chip${state.not_added_only ? ' active' : ''}" style="${lidarrCacheLoaded ? '' : 'opacity:.5'}"
              ...
              <input type="checkbox" ${state.not_added_only ? 'checked' : ''} onchange="setBacklogNotAddedOnly('${svc}', this.checked)" ${lidarrCacheLoaded ? '' : 'disabled'}> Not in Lidarr yet</label>
          </div>
        </div>
        ${pending > 0 && !state.only_resolved ? '<p class="fs-11 t3 mt-8" style="font-style:italic">Pending artists are still being looked up by Phase 1.4 of the music pipeline; they become addable once their MusicBrainz ID is resolved.</p>' : ''}
...
function useStarter(id, btn) {
  ...
  pendingDiscussContext = { kind: 'starter', starter_id: id };
  _setDiscussBanner('Curator opened this thread');
  const inp = document.getElementById('chat-input');
  inp.value = '';
  inp.focus();
}
function discussLastPlayed(i) {
  const e = _lastPlayedEntries[i];
  if (!e) return;
  const isMusic = e.media_type === 'music';
  let title = e.title;
  if (isMusic && e.series_title) title = `${e.title} — ${e.series_title}`;
  ...
  // trust model + sticky pendingDiscussContext pipeline as discussDeletion/
...
async function recomputeTaste(statusId = null) {
...
async function syncHistory(force = false, statusId = null) {
  const status = statusId ? document.getElementById(statusId) : null;
  const el = document.getElementById('history-stats');
...
async function reattributeHistory(btn) {
...
async function loadDepsStatus(btn) {
...
const INTEGRATION_CARDS = [
  {key: 'plex', title: 'Plex', test: 'plex', fields: [
    ...
    {id: 'plex_token', label: 'Auth token', secret: true, required: true},
  ]},
  ...
  {key: 'music', title: 'Music metadata', fields: [
    ...
    {id: 'listenbrainz_token', label: 'ListenBrainz user token', secret: true},
...
function renderIntegrationCard(card, cfg) {
...
function _integrationChanges(card) {
  const changes = {};
...
async function saveIntegrations(cardKey, btn) {
...
async function clearIntegrationSecret(fieldId, cardKey, btn) {
...
async function testIntegration(cardKey, btn) {
  const msg = document.getElementById(`int-msg-${cardKey}`);
...
async function rebuildModels(cardKey, btn) {
  const msg = document.getElementById(`int-msg-${cardKey}`);
...
async function testArr(svc, btn) {
...
async function saveArrConfig(svc, btn) {
  const url = document.getElementById(`arr-url-${svc}`).value.trim();
  const key = document.getElementById(`arr-key-${svc}`).value.trim();
  const msg = document.getElementById(`arr-msg-${svc}`);
...
async function loadArrProfiles(svc, btn) {
...
async function saveArrDefaults(svc, btn) {
...
async function _setNotifPref(toggle) {
...
async function submitPinSet(btn) {
...
function _renderHistoryStats(s) {
...
function _laneParam() {
  return (currentRecsLane === 'library' || currentRecsLane === 'discovery') ? `&lane=${currentRecsLane}` : '';
}
...
function _pollRecsUntilFresh(cat, btn, prevStamp) {
  _stopRecsPoll();
      ...
      <div class="grow">
        <div class="panel-item-head">
          ...
          <div class="panel-actions">
            ...
            ${rec.lane === 'discovery' && currentUser?.is_admin
...
async function onAddRecToArr(btn) {
...
async function searchLibrary() {
  ...
  try {
    const cat = currentRecsCategory ? `&category=${currentRecsCategory}` : '';
    const r = await api(`/api/library/semantic-search?q=${encodeURIComponent(q)}${cat}&limit=12`);
    const hits = r.results || [];
    const back = `<div class="mb-12"><button type="button" class="btn btn-secondary btn-sm" onclick="loadRecs(currentRecsCategory)">← Back to recommendations</button></div>`;
    if (!hits.length) {
    ...
    const covBanner = (cov && cov.constraints >= 3 && cov.best_met <= 1)
      ...
      : '';
    el.innerHTML = back +
      `<div class="fs-13 t2 mb-12">Semantic matches in your library for <b class="t-amber">${esc(q)}</b>${currentRecsCategory ? ` · ${esc(currentRecsCategory)}` : ''}${anchorTag}${modeTag}</div>` +
      covBanner +
      hits.map(h => `
      <div class="card mb-8">
        <div class="panel-item-head">
...
function _renderDeletionProposals(proposals) {
  ...
  if (!proposals?.length) {
    el.innerHTML = emptyHtml('No proposals yet — <b>Analyse library</b> asks the curator for deletion candidates.', 'Analyse library', 'loadDeletions(currentDelCategory,null,true)');
    updateDelBulkCount();
    return;
  }
...
function onFixMatch(btn) {
  ...
  openMatchPicker({service, arrId: Number(mediaid), title, category: category || undefined,
                   onDone: () => { _delProposalsAll = null; loadDeletions(currentDelCategory); }});
}
...
async function liftProtection(id, btn) {
...
async function setPrinciple(id, action, btn) {
  if (action === 'reject') {
...
async function condensePrinciples(btn) {
...
async function downscaleDone(id, btn) {
  btnBusy(btn);
  try {
...
function _deleteBody(what, note) {
...
function _syncDelPosterVisual(cb) {
...
function _setDiscussBanner(text) {
  ...
  // B4: every set/clear of the banner already means pendingDiscussContext
  ...
  _setChatWatermark(pendingDiscussContext?.poster_url || null);
}
...
function _setChatWatermark(url) {
  const msgs = document.getElementById('messages');
  if (!msgs) return;
...
function discussDeletion(id, title, pitch, category, poster_url) {
...
async function loadMappingStats(btn) {
...
async function checkMappingCoverage(btn) {
...
function showKbTab(name) {
  _kbTab = name || 'overview';
  document.querySelectorAll('#kb-tabs .cat-tab').forEach(b => b.classList.toggle('active', b.dataset.kbtab === _kbTab));
  document.querySelectorAll('#enrich-view .kb-tab').forEach(s => { s.hidden = s.dataset.kbtab !== _kbTab; });
  if (_kbTab === 'overview' || _kbTab === 'maintenance') loadEnrichStatus();
  if (_kbTab === 'attention') loadKbAttention(_kbAtt.offset, _kbAtt.reason);
  if (_kbTab === 'music') loadMusicStatus();
  if (_kbTab === 'profiles' && !_kbLoaded.has('profiles')) loadProfiles();
  _kbLoaded.add(_kbTab);
}
...
async function loadCacheInventory(btn) {
  btnBusy(btn, 'Loading…');
  try {
...
const PIN_ID_KINDS = ['tmdb_id', 'tvdb_id', 'imdb_id', 'anilist_id', 'mal_id', 'mbid'];
...
function _kbSourceChips(sources) {
...
function _kbItemCard(it, extraBadges) {
  ...
  const findings = (it.findings || []).map(f => `<div class="panel-item-meta row mt-4">
      ...
      ${currentUser?.is_admin ? `<button type="button" class="btn btn-secondary btn-sm" data-fid="${Number(f.id)}" onclick="kbDismissFinding(this)" title="Silence this finding for good — the audit will not raise it again">Dismiss finding</button>` : ''}
    </div>`).join('');
  return `<div class="panel-item">
    <div class="panel-item-head">
...
function kbDismissFinding(btn) { return _kbAction(btn, `/api/enrichment/findings/${Number(btn.dataset.fid)}/dismiss`, 'POST', 'Finding dismissed — the audit will not raise it again'); }

// The list behind a number: a section under the overview table with the
// state explainer as its hint, a pager and a Close.
async function loadKbItems(cat, states, offset = 0) {
...
async function loadKbAttention(offset = 0, reason = null) {
...
async function _kbAction(btn, path, method, doneText) {
  btnBusy(btn);
  try {
...
function kbRetry(btn)    { return _kbAction(btn, `/api/enrichment/items/${encodeURIComponent(btn.dataset.svc)}/${Number(btn.dataset.id)}/retry`, 'POST', 'Attempts reset — the next run tries again'); }
function kbIgnore(btn)   { return _kbAction(btn, `/api/enrichment/items/${encodeURIComponent(btn.dataset.svc)}/${Number(btn.dataset.id)}/ignore`, 'POST', 'Ignored — no more retries, not counted as open'); }
function kbUnignore(btn) { return _kbAction(btn, `/api/enrichment/items/${encodeURIComponent(btn.dataset.svc)}/${Number(btn.dataset.id)}/ignore`, 'DELETE', 'Back in the queue'); }

function kbFixMatch(btn) {
...
async function renderMatchPicker(box, opts) {
...
async function _pickerPost(btn, payload, doneText, keepOpen = false) {
  btnBusy(btn);
  try {
...
function _pickerBase(btn) {
  const {svc, id, cat, title} = btn.dataset;
  const p = {service: svc, arr_id: Number(id), title: title || undefined};
  if (cat) p.category = cat;
  return p;
}
function pickerPin(btn)    { return _pickerPost(btn, Object.assign(_pickerBase(btn), JSON.parse(btn.dataset.ids || '{}')), 'Pinned — re-enriches on the next run'); }
function pickerReject(btn) { return _pickerPost(btn, Object.assign(_pickerBase(btn), {rejected: [JSON.parse(btn.dataset.ids || '{}')]}), 'Excluded for good', true); }
function pickerFreePin(btn) {
...
async function removeFixMatch(btn) {
  const c = _pickerCtx;
  if (!c) return;
  btnBusy(btn);
  try {
...
async function loadBackfillPanel() {
...
async function startBackfill(source, btn) {
  btnBusy(btn, 'Starting…');
...
async function stopBackfill(source, btn) {
...
function _musicPhase(p) {
  const f = MUSIC_PHASES[p.phase || ''];
  return f ? f(p) : ['Starting…', 5];
}

async function startMusicPipeline(btn) {
...
async function stopMusicPipeline(btn) {
  btnBusy(btn, 'Stopping…');
...
// result, a load error).
function _enrichCats() { return ['music','movie','show','anime'].filter(c => document.getElementById(`enrich-cat-${c}`)?.checked); }
function _enrichSrc()  { return document.querySelector('input[name="enrich-src"]:checked')?.value || 'watch_history'; }

async function startEnrichNew(btn) {
...
async function computeTaste(btn) {
  const el = document.getElementById('enrich-progress-area');
...
function _renderLibraryConfig([cfg, disc]) {
...
async function saveLibraries(btn) {
  const libraries=discoverSections.map(s=>({
    key:s.key, title:s.title, plex_type:s.type,
...
// ── ORPHAN REPAIR ─────────────────────────────────────────────────────────────
async function checkOrphans(btn) {
...
async function _classifyProcess(name, isGame) {
  try {
