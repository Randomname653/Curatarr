...
async def main():
    ...
    # Simulate user
    user_pin = "MySecurePin123"
    user_id = "plex_user_123"
    
    # Derive key
    key, salt = encryptor.derive_key(user_pin, user_id)
    print(f"Derived key (hex): {key.hex()}")

    # Encrypt data
    sensitive_data = {
        "genre_affinity": {"Action": 0.9, "Romance": 0.1},
        "watch_history_summary": {"total_hours": 500, "completion_rate": 0.85}
    }

    encrypted = encryptor.encrypt_json(sensitive_data, key, salt)
    print(f"Encrypted: {encrypted[:50]}...")

    # Decrypt — re-derive key using stored salt
    stored_salt = base64.b64decode(json.loads(encrypted)["salt"])
    key2, _ = encryptor.derive_key(user_pin, user_id, salt=stored_salt)
    decrypted = encryptor.decrypt_json(encrypted, key2)
