...
def _restrict_env_acl(path) -> None:
    """Windows: chmod is a no-op and NTFS inherits the parent DACL, so on a
    ...
    try:
        ...
        subprocess.run(  # nosec B603 B607 - fixed argv, no shell, path is ours
...
def _plain(value) -> str:
    ...
    pydantic ``SecretStr`` renders as ``**********`` under str()/f-strings -
    a caller that forgets ``.get_secret_value()`` would persist the mask as
    the real secret. Unwrap here too, and refuse to write a mask outright.
    """
    if value is None:
        return ""
    ...
    if text and set(text) == {"*"}:
        raise ValueError("refusing to write a masked placeholder as a secret")
    return text


_ENV_FORBIDDEN_CHARS = ("\r", "\n", "\x00")
DEFAULT_PLEX_CLIENT_ID = "Curatarr"


def _plex_client_id(config: dict) -> str:
    """The X-Plex-Client-Identifier this install presents to plex.tv.

    Every install used to share the constant "Curatarr", so one install's
    PINs were pollable under another's identity. A FRESH install (no JWT
    secret yet - this write is its first) gets a random per-install id.
    An established one keeps whatever it has: its users' stored Plex tokens
    and the device entry on plex.tv were issued to that id, and changing it
    under them would buy nothing but a stray new device and possible
    re-authentication for every household member.
    """
    given = (config.get("plex_client_id") or "").strip()
    if given:
        return given
    live = _live_settings()
    current = (getattr(live, "PLEX_CLIENT_ID", "") or "").strip() or DEFAULT_PLEX_CLIENT_ID
    established = bool(_plain(config.get("jwt_secret")) or live.effective_jwt_secret)
    if established or current != DEFAULT_PLEX_CLIENT_ID:
        return current
    return f"curatarr-{secrets.token_hex(16)}"


def write_env(config: dict) -> None:
    """Write a validated config dict to .env file."""
    # Carry the live secret forward: the wizard's request never includes it,
    # and regenerating it on every re-run logged the whole household out.
    jwt_secret = (_plain(config.get("jwt_secret"))
                  or _live_settings().effective_jwt_secret
                  or secrets.token_hex(32))
    plex_client_id = _plex_client_id(config)
    ...
    lines = [
        ...
        "",
        "# Plex",
        f"PLEX_CLIENT_ID={plex_client_id}",
        f"PLEX_URL={config.get('plex_url', '')}",
        f"PLEX_TOKEN={_plain(config.get('plex_token', ''))}",
        "",
        "# Ollama",
        ...
        # operator's tuned interval/binge thresholds to the defaults.
        f"SYNC_ON_STARTUP={'true' if _live_settings().SYNC_ON_STARTUP else 'false'}",
        f"SYNC_INTERVAL_HOURS={_live_settings().SYNC_INTERVAL_HOURS}",
        "",
        "# Binge detection",
        f"BINGE_EPISODE_THRESHOLD={_live_settings().BINGE_EPISODE_THRESHOLD}",
        f"BINGE_SESSION_HOURS={_live_settings().BINGE_SESSION_HOURS}",
        f"BINGE_SERIES_PERCENT={_live_settings().BINGE_SERIES_PERCENT}",
    ]
    ...
    # Values arrive from request bodies; one carrying a line break would
    # write a key of its caller's choosing (PLEX_TOKEN="x\nPLEX_LOGIN_
    # REQUIRE_MEMBERSHIP=false"). No legitimate URL, token or model name
    # contains one, so refuse the whole write rather than strip silently.
    for line in lines:
        if any(ch in line for ch in _ENV_FORBIDDEN_CHARS):
            key = line.split("=", 1)[0].strip() or "value"
            raise ValueError(f"{key}: line breaks and NUL are not allowed in settings values")
