...
| Session revocation | `User.token_version`, JWT `ver` claim, `TokenRefreshMiddleware._load_refresh_subject` | `get_current_user` enforces `is_active` and a version match; logout/deactivation bump the version. The rolling refresh re-issues from the DB row, never from stale claims. Pre-2026-09 tokens read as version 0 and stay valid. |
| First-run setup code | `auth.SETUP_CODE`, `_require_setup_code`, printed by `main.lifespan` | Pre-admin setup mutations require a console-printed one-time code unless the caller is localhost; the frontend `api()` prompts for it on a 401 from `/api/setup/*` and replays. |
