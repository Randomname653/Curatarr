...
| Tech-profile liveness | `plex_sync._prune_stale_tech_profiles`, `plex_sync._LAST_FETCH_COMPLETE`, `size_norms._copy_fingerprint`, `chat._check_protection_intent_bg` | A MediaTechProfile row lives only as long as Plex has its rating key: the tech sync prunes the rest, but only for categories whose sections were fetched completely and non-empty (an outage never reads as an emptied library). Identical files under different keys count as one copy in the DUPLICATE note and the redundancy audit. The intent hook's real outcome (protected; watchlist added or refused; downscale flagged or not) is stored as a ProactiveMessage for the bell, because the reply was streamed before the hook ran; the constitution and DISCUSSION_UI_BLOCK say the curator executes nothing itself. 2026-09-13, the Armor Shop case. |
...
| Login membership + owner-first | `auth._assert_plex_membership`, `PLEX_LOGIN_REQUIRE_MEMBERSHIP` | Any plex.tv account used to become a user (the first one admin). New accounts must now appear in the server's `/accounts`; the first must be the token owner (`plex.tv/api/v2/user` with the server token). Fails closed for new accounts only. |
| Session revocation | `User.token_version`, JWT `ver` claim, `TokenRefreshMiddleware._load_refresh_subject` | `get_current_user` enforces `is_active` and a version match; logout/deactivation bump the version, and since 2026-09-25 so does `DELETE /api/users/{id}`, so re-enabling a deleted account does not revive its old tokens; the setup gate (`require_admin_or_first_run`) goes through the same `_user_from_bearer` instead of decoding the JWT itself. The rolling refresh re-issues from the DB row, never from stale claims. Pre-2026-09 tokens read as version 0 and stay valid. |
| First-run setup code | `auth.SETUP_CODE`, `_require_setup_code`, printed by `main.lifespan` | Pre-admin setup mutations require a console-printed one-time code unless the caller is localhost; the frontend `api()` prompts for it on a 401 from `/api/setup/*` and replays. Wrong codes are budgeted per address and globally (10 / 100 per 15 minutes, then 429; localhost exempt) (2026-09-25). |
| .env write discipline | `setup_wizard._plain`, `write_env` | Unwraps `SecretStr` and refuses `**********` (library_configure once passed the raw objects — a forgeable JWT secret waiting to happen); carries the live JWT secret and tuned SYNC/BINGE values forward; atomic temp+replace; Windows ACL narrowed via icacls. |
...
| Late arrivals find their history | `src/services/music_rematch.py` (`arrivals`, `rematch_arrivals`), phase 1b of `run_music_pipeline`, app_state `music_rematch_added_at:<uid>` | Music that lands later still claims the plays waiting for it (2026-09-21). The forward match gives each play one attempt ever — a cursor bought that after the old full re-scan froze the app for about a minute — so a library that grows by trickle could never improve its match rate. Measured, the premise was wrong: the minute came from materialising 203k ORM rows; three columns and the same normalisation take 0.99 s. So the match runs the other way, asking each newly arrived track whether the history was waiting for it, off the local Plex track index the lyrics collector refreshes daily — no extra Plex call, and a quiet run is one indexed query. First pass on the owner's data: 686 tracks claimed 5,842 historic plays in 1.1 s. |
| Music-pipeline miss memory | `src/services/music_misses.py` (`due_filter`, `record_miss`, `clear_miss`, `stats`), `MusicLookupMiss`, `music_matcher.resolve_artist_mbids`, the custodian's `_run_spotify` done-check | A lookup that found nothing is a result (2026-09-20). Phase 1.4 selected on "artist_mbid IS NULL" and wrote nothing on a miss, so the owner's 1,696 unresolvable Spotify artists were re-queried against MusicBrainz every night at ~1 req/s — seventeen minutes a day for a yield near zero, and the done-check read successes so it stamped "drained" each time. Misses now go in their own table with a growing wait (14, 30, 90 days; the empty-string trick the genre phase uses is unavailable because readers treat a non-null id as resolved), the selection skips what is still waiting, a later success clears the record, and the done-check counts attempts. The Music-pipeline tab names the parked count so a bar short of 100 % explains itself. |
...
| Series editions | `src/services/editions.py` (`classify_files`, `anidb_flags`, `sync_editions`, `edition_line`, `upgrade_rows`, `check_releases`), `SeriesEdition`, custodian task `editions_sync`, `SonarrClient.get_episode_files` / `search_releases`, `GET /api/library/editions/{id}/releases` | Which cut is on disk, and is there an uncensored one (2026-09-15)? Owned from Sonarr's episode files: the parsed quality source of every file (Blu-ray / DVD against web / television) and the names and custom formats that say uncensored / uncut / unrated or censored. Exists from the AniDB tags of the offline snapshot, by AniDB's definitions: "tv censoring" (147 of the owner's series) means the disc release is the uncensored cut, "censored uncensored version" (32) means even the disc keeps censoring. Walker looked at daily (custodian cadence 24 h) with a weekly per-row recheck, one call per series at 60 requests a minute, 200 series per tick; the verified block carries an Edition line for shows and anime, the Curation upgrade list shows "TV cut — uncensored disc release exists" (broadcast or web files on disk while AniDB says the disc is uncensored) with a Search-releases button that asks Sonarr's indexers live (seasons 1–2, uncensored by name or from a Blu-ray/DVD source). |
| Untrusted HTML | `ui.sanitizeHtml` / `renderMarkdown`, `tests/test_untrusted_html.py` | Model output reaches innerHTML through one sanitizer with data attributes off and the dispatcher's own attributes stripped by name (2026-09-25). DOMPurify's defaults kept `data-on-error`, and app.js runs any registered action named there in the capture phase, so an `<img>` in a curator reply could have approved a deletion without a click; the action handlers now encode their path arguments too, and `esc()` escapes quotes. |
| Plex PIN binding, setup-code budget | `auth._bind_pin` / `_check_pin_binding`, `auth.SETUP_CODE_FAILS_*`, `setup_wizard._plex_client_id` | `/plex/pin` hands its creator a nonce that `/plex/poll` must echo, polls are budgeted per client address, wrong setup codes per address and globally, `write_env` refuses values with line breaks, and a fresh install gets its own random `PLEX_CLIENT_ID` while established ones keep theirs (2026-09-25). |
| Deletion claim + verify | `recommendations._transition_if_open`, `_blocked_reason`, `_protection_for`, `_verify_arr_item_gone`, `episodic_memory._ground_protection_actions` | Status and ProtectedMedia are re-read before the Lidarr guard and again right before the DELETE; the row is claimed with a conditional UPDATE committed at once, so one proposal sends one DELETE ever; a delete that times out (read timeout now 300 s) is checked by GET and parks in limbo when unclear; Keep is refused on a closed row; Plex-music proposals get a real `/identity` probe; the protection classifier acts only on the anchor title or titles in the user's own words, three per turn; boot parks interrupted `deleting` rows in limbo (2026-09-25). |
| Sync watermark, job contract | `plex_sync._run_plex_sync` (`sync_started`), `scheduler._tracked`, `connection._ensure_deletion_proposal_indexes` | `last_sync_at` is the moment Plex was asked, not the finish time, so a play landing mid-sync is not lost; a scheduler job returning `False` is not stamped (failed syncs, busy skips and game-time waits retry next tick, and a mere wait creates no Activity card); the per-entry DB walk, VACUUM and the backup run in worker threads; the Pass-90c rebuild had lost the two `deletion_proposals` indexes, recreated at every start (2026-09-25). |
| Chat runtime guards | `llm_priority.curator_start`, `chat._send_message_guarded`, `_release_unstarted_chat`, `proactive_messages.get_unread_messages(seen=)` | A cancel during the eviction undoes the slot acquire; the deletion funnel releases only a slot it holds; the one-reply-in-flight guard is taken before anything is saved or built and released on every exit, a never-started stream included; Respond leaves a verification question unread until it is answered; the bell's background poll is not an impression (2026-09-25). |
| Member view, budgets, parsers | `tasks._visible` / `_owner_of`, `library_status`, `media_enricher.omdb_quota_left`, `episodic_memory._split_action_line`, `image_proxy._host_resolves_public` | The 2026-09 audit residuals, closed 2026-09-25: a member's Activity view and task stream carry only tasks whose id names them; the library status hides arr addresses, root folders and test results from members; OMDb calls are counted per UTC day in app_state and stop at `OMDB_DAILY_LIMIT` (OMDb's own limit answer marks the day spent); the protection classifier's action line splits only before a known field label, so a title with a pipe stays whole; the image proxy resolves a whitelisted name and refuses private, loopback and link-local answers before every upstream connection. |
| Vector identity | `ChromaDBWrapper.upsert_documents` / `use_collection`, `taste_engine._doc_is_item`, `enrichment._clear_emb_caches`, `media_enricher.ensure_verified_data` | Chroma's `add` keeps the old vector for an existing id without raising, so re-enrichment upserts; a stored vector is used only when its doc's domain and title match the item (doc ids mix TMDB movie, TMDB TV and AniList number spaces); a force re-enrich clears only the requested items' embedding caches; the wrapper switches collection in place after a profile flip; `ensure_verified_data` keeps the ids it resolved for every top-up (2026-09-25). |
...
- `.github/workflows/` — tests, CodeQL, Bandit, OSV-Scanner, Scorecard and
  ...
  both OSV jobs on the same scan-args (`--no-resolve`; `--config` so the
  exceptions reach `lock/requirements.txt`, the tested install's closure).
